   ...
   context(@"on error free flow", ^{
       context(@"on find device", ^{
           ...
           it(@"creates device object", ^{
               [[expectFutureValue(foundDevice) shouldEventually] equal:[TestToolBox objectFromJSONFromFile:@"device.json"]];
           });
       });
       ...
       context(@"on update device", ^{
           ...
           it(@"creates device object", ^{
                [[expectFutureValue(updatedDevice) shouldEventually] equal:[TestToolBox objectFromJSONFromFile:@"device.json"]];
           });
       ...
       context(@"on delete device", ^{
           ...
           it(@"creates device object", ^{
                [[expectFutureValue(deletedDevice) shouldEventually] equal:[TestToolBox objectFromJSONFromFile:@"device.json"]];
           });
       });
   });
});
