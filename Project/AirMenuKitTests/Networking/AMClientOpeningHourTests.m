    ...
    describe(@"on error free flow", ^{
        context(@"on find opening hour", ^{
            ...
            it(@"creates opeing hour object", ^{
                [[expectFutureValue(foundHour) shouldEventuallyBeforeTimingOutAfter(5.0)] equal:[TestToolBox objectFromJSONFromFile:@"opening_hour.json"]];
            });
        });
        ...
        context(@"on update opening hour", ^{
            ...
            it(@"creates opeing hour object", ^{
                [[expectFutureValue(updatedHour) shouldEventuallyBeforeTimingOutAfter(5.0)] equal:[TestToolBox objectFromJSONFromFile:@"opening_hour.json"]];
            });
        ...
        context(@"on delete opening hour", ^{
            ...
            it(@"creates opeing hour object", ^{
                [[expectFutureValue(deletedHour) shouldEventuallyBeforeTimingOutAfter(5.0)] equal:[TestToolBox objectFromJSONFromFile:@"opening_hour.json"]];
            });
        });
    });
});
